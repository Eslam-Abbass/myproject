...
exports.signup =catchAsyncErorr(
  async (req, res,next) => {
    ...
    if (user) {
      next(new appErorr(`Email Aleardy Exist`,400))
    ...
    if (user) {
      ...
      if (match) {
        ...
        if (user.emailConfirm == true) {
          ...
          next(new appErorr(`Verfiyed Email First!`,400))
         
        }
      } else {
        next(new appErorr(`In_valid password`,400))
      }
    } else {
      next(new appErorr(`In_valid account`,400))
    }
  }
) 
